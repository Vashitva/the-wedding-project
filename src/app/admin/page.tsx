  ...
  return (
    <main id="main" className="mx-auto max-w-6xl px-5 py-16">
          ...
          <ul className="card divide-y divide-line">
            {stats.perEvent.map((e) => (
              <li key={e.eventId} className="flex justify-between px-5 py-3">
                <span>{e.name}</span>
                <span className="tabular-nums text-olive">{e.attending}</span>
              </li>
            ))}
          </ul>
        </div>
          ...
          <ul className="card divide-y divide-line">
            {stats.perMeal.map((m) => (
              <li key={m.mealId} className="flex justify-between px-5 py-3">
                <span>{m.name}</span>
                <span className="tabular-nums text-olive">{m.count}</span>
              </li>
            ))}
          </ul>
        </div>
                ...
                <div className="px-5 pb-5 text-sm">
                  <ul className="space-y-1.5">
                    {rsvp.responses.map((r) => (
                      <li key={r.guestId} className="flex flex-wrap gap-x-3">
                        <span className={r.attending ? "text-olive" : "text-ink-faint"}>
                          {r.attending ? "✓" : "✕"}
                        </span>
                        <span>{r.name}</span>
                        {r.attending && (
                  ...
                  <dl className="mt-4 space-y-1 text-ink-soft">
                    <div className="flex gap-2">
                      ...
                      <dd>
                        <a href={`mailto:${rsvp.email}`} className="hover:text-olive">
                          {rsvp.email}
                        </a>
                      </dd>
                    </div>
      ...
      <p className="mt-12 text-center text-sm">
        <Link href="/" className="text-ink-faint hover:text-olive">
          ← Back to the site
        </Link>
      </p>
    </main>
