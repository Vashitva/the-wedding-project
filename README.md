...
The first screen is a white frame: backdrop, veil, vignette and a fine grain,
with the names racking into focus and drifting away as you scroll. The veil
lifts the flowers back rather than darkening them, so the page never turns
grey, and the frame ends on a clean cut into the page below.

It ships with a backdrop already in place: `public/hero.png` — a wall of white
flowers thrown out of focus, with blush blooms and sage leaves behind them.
...
Regenerate it after editing the palette or the planes of flowers:
